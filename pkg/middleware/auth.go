...
func AuthorizationMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			server.ErrResp(c, http.StatusUnauthorized, "token required", nil)
			c.Abort()
			return
		}
		...
		if len(token) < 7 || token[:7] != "Bearer " {
			server.ErrResp(
				c, http.StatusUnauthorized, "invalid token format", &server.Extras{"invalid_token": token})
			c.Abort()
			return
		}
		...
		if err != nil {
			server.ErrResp(
				c, http.StatusUnauthorized, "invalid token", &server.Extras{"invalid_token": token})
			c.Abort()
			return
		}
